...
//Creates an interface with the CollateralPolicy contract.
interface CollateralPolicyInterface {
...
//The CollateralDepository contract holds all the funds for the customers who purchased an insurance policy
//for their collateral. If a customer files a succesful claim, the payout will come from this contract.
contract CollateralDepository  {

    address public owner;

    //Sets the owner of the CollateralDepository contract to be the address which deployed it.
    constructor(){
        owner = msg.sender; 
    }
    ...
    //This function is called by the customer (from the CollateralPolicy contract via an interface) when they want to file
    //a claim. The claim info entered by the customer is used to create a claim struct in this CollateralDepository contract. 
    function processClaim(address _policyId, uint _amountLiquidated, address _liquidatedContract, address _collateralContract, address payable _payoutAddress, uint _payoutAmount) external {
         
         //This creates a new instance of a claim struct.
         Claim memory newClaim = Claim(claimsArray.length +1, _policyId, block.timestamp, _amountLiquidated, _liquidatedContract, _collateralContract, _payoutAmount, _payoutAddress, false, false, 0);
    ...
    function approveClaim(address _policyId) external onlyOwner {
        ...
        //This require statement ensures that the claim is being filed for the contract that was insured. 
        require(claimsRegistry[_policyId].liquidatedContract == claimsRegistry[_policyId].collateralContract, "Claim must be for insured collateral");
        ...
        CollateralPolicyInterface collateralPolicyInstance = CollateralPolicyInterface(_policyId); 
        ...
        //Calls the "updateCoverage" function in the customer's CollateralPolicy contract and sets the coverage left to 0.
        //This is because collateral may be reimbursed only once. 
        collateralPolicyInstance.updateCoverage();
    }
    ...
    function viewClaimCustomer(address _policyId) external view returns (uint, address, uint, uint, address, address, uint, address, bool, bool, uint){
        ...
        return (
            ...
            claimsRegistry[custPolicy].amountLiquidated,
            claimsRegistry[custPolicy].liquidatedContract,
            claimsRegistry[custPolicy].collateralContract,
    ...
    //This function allows the Owner/Administrator to see how much total ETH has been deposited into the Collateral Depository contract.
    ...
    //Allows the Collateral Depository contract to receive ETH from the CollateralPolicyFactory.
    ...
    /*
      1. This function allows the Owner/Administrator to deposit ETH from the Collateral 
      ...
      2. To do so, the "mint()" function is called. This automatically sends
         cETH back to the Collateral Depository.
    */ 
    ...
    //Collateral Depository contract. 
    ...
    /*
      1. This function allows the Owner/Administrator to deposit cETH from the Collateral 
          Depository into the cETH contract (Compound Finance) on the Goerli testnet.
      2. To do so, the "redeem()" function is called. This automatically sends
         ETH back to the Collateral Depository.
    */ 
